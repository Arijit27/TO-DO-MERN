...
function App() {
  ...
  useEffect(() => {
    if (token) {
      axios
        .get("https://backend-todo-afm8.onrender.com/tasks", {
  ...
  const addTask = () => {
    axios
      .post(
        "https://backend-todo-afm8.onrender.com/tasks",
        { title: newTask },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      .then((response) => {
  ...
  const deleteTask = (id) => {
    ...
    setTimeout(() => {
      axios
        .delete(`https://backend-todo-afm8.onrender.com/tasks/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
  ...
  const toggleCompletion = (id) => {
    ...
    axios
      .put(
        `https://backend-todo-afm8.onrender.com/tasks/${id}`,
            ...
            <ul className="task-list">
              {tasks.map((task) => (
                <li
                  key={task._id}
                  className={`task-item ${
                    deletingTaskId === task._id ? "fade-out" : ""
                  }`}
                >
